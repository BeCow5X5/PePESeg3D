...
from diff_gaussian_rasterization_cream import GaussianRasterizationSettings as GaussianRasterizationSettingsCream, GaussianRasterizer as GaussianRasterizerCream

def render_with_cream(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, override_mask = None, filtered_mask = None):
    """
    Render the scene. 
    ...
    raster_settings = GaussianRasterizationSettingsCream(
        image_height=int(viewpoint_camera.image_height),
        image_width=int(viewpoint_camera.image_width),
        tanfovx=tanfovx,
        tanfovy=tanfovy,
    ...
    rasterizer = GaussianRasterizerCream(raster_settings=raster_settings)
    ...
    # Create dummy uncertainty tensor (not used, but required by rasterizer)
    uncertainty = torch.zeros((means3D.shape[0], 1), dtype=means3D.dtype, device=means3D.device)
    ...
    # Rasterize visible Gaussians to image, obtain their radii (on screen). 
    rendered_image, rendered_depth, rendered_uncertainty, radii = rasterizer(
        means3D = means3D,
        means2D = means2D,
        shs = shs,
        colors_precomp = colors_precomp,
