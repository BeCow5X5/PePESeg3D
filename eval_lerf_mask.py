...
# Multi-scale rendering and evaluation script for LERF mask benchmark
...
def render_and_evaluate_scale(scale_value, view_data, TEXT_PROMPT, text_mask, gt_masks_dict,
    ...
    # ========== Extract query centroids for THIS scale ==========
    with torch.no_grad():
        ...
        if use_hdbscan:
            ...
            if cluster_centroids_0 is None:
                ...
                text_mask_tensor = torch.from_numpy(text_mask).cuda() if isinstance(text_mask, np.ndarray) else text_mask
                mask_bool = text_mask_tensor > 0
                ...
                # Find overlapping clusters with text mask
                text_mask_np = text_mask if isinstance(text_mask, np.ndarray) else text_mask.cpu().numpy()
                
                # Resize text_mask to match cluster_labels size if needed
                if text_mask_np.shape != cluster_labels_0.shape:
                    from scipy.ndimage import zoom
                    zoom_factor = (cluster_labels_0.shape[0] / text_mask_np.shape[0],
                                   cluster_labels_0.shape[1] / text_mask_np.shape[1])
                    text_mask_resized = zoom(text_mask_np.astype(float), zoom_factor, order=0) > 0.5
                else:
                    text_mask_resized = text_mask_np
                
                mask_bool = text_mask_resized > 0
...
def render_set_multiscale(model_path, name, iteration, views, gaussians, feature_gaussians, 
                          pipeline, background, scale_gate, groundingdino_model, sam_predictor,
                          TEXT_PROMPT, gt_folder_path, threshold=0.9, smooth_K=16, use_hdbscan=True):
    """
    ...
    pred_obj_path = os.path.join(model_path, name, f"ours_{iteration}_text", "test_mask")
    makedirs(render_path, exist_ok=True)
    makedirs(gts_path, exist_ok=True)
    makedirs(colormask_path, exist_ok=True)
    makedirs(pred_obj_path, exist_ok=True)
    
    # Load GT masks for this text prompt
    gt_masks_dict = {}
    for idx, view in enumerate(views):
        gt_mask_path = os.path.join(gt_folder_path, f'{idx}', f'{TEXT_PROMPT}.png')
        print(gt_mask_path)
    ...
    # Get text mask from Grounded-SAM on first frame
    with torch.no_grad():
        view0 = views[0]
        results = render_contrastive_feature(view0, feature_gaussians, pipeline, background,
                                            norm_point_features=True, smooth_weights=None,
                                            smooth_type='traditional', smooth_K=smooth_K)
        rendered_features0 = results["render"]
        rendered_features0 = F.interpolate(rendered_features0.unsqueeze(0),
                                          view0.original_image.shape[1:],
                                          mode='bilinear').squeeze(0)
        
        rendering0 = view0.original_image
        image = (rendering0.permute(1,2,0).cpu().numpy() * 255).astype('uint8')
    
    text_mask, annotated_frame_with_mask = grouned_sam_output(
        groundingdino_model, sam_predictor, TEXT_PROMPT, image,
        filter_phrase=None, exclude_phrase=None
    )
    Image.fromarray(annotated_frame_with_mask).save(
        os.path.join(render_path[:-8], f'grounded-sam---{TEXT_PROMPT}.png'))
    ...
    for scale_value in test_scales:
        ...
        mean_iou, mean_biou, temp_masks = render_and_evaluate_scale(
            scale_value, view_data, TEXT_PROMPT, text_mask, gt_masks_dict,
    ...
    if best_masks is not None:
        ...
        for idx, view in enumerate(tqdm(views, desc="Saving best masks")):
            pred_obj_img_path = os.path.join(pred_obj_path, f'{idx:05d}')
            ...
            # Save GT image (once per view)
            if idx < len(views):
                gt = view.original_image[0:3, :, :]
                torchvision.utils.save_image(gt, os.path.join(render_path, f'{idx:05d}.png'))
                torchvision.utils.save_image(gt, os.path.join(gts_path, f'{idx:05d}.png'))
    
    # Save results summary
    summary_path = os.path.join(model_path, name, f"ours_{iteration}_text", "results_summary.txt")
...
def render_sets(dataset: ModelParams, iteration: int, pipeline: PipelineParams, 
                skip_train: bool, skip_test: bool, gt_folder_path: str,
                smooth_K: int = 16, use_hdbscan: bool = True):
    """Main function for multi-scale rendering and evaluation"""
    with torch.no_grad():
        dataset.eval = True
        ...
        # Load Grounding DINO and SAM
        ckpt_repo_id = "ShilongLiu/GroundingDINO"
        ckpt_filenmae = "groundingdino_swinb_cogcoor.pth"
        ckpt_config_filename = "GroundingDINO_SwinB.cfg.py"
        groundingdino_model = load_model_hf(ckpt_repo_id, ckpt_filenmae, ckpt_config_filename)
        
        sam_checkpoint = os.environ.get('SAM_CKPT', './third_party/segment-anything/sam_ckpt/sam_vit_h_4b8939.pth')
        ...
        for TEXT_PROMPT in positives:
            mean_iou, mean_biou, best_scale = render_set_multiscale(
                ...
                pipeline, background, scale_gate, groundingdino_model, sam_predictor,
                TEXT_PROMPT, gt_folder_path, threshold=0.7, smooth_K=smooth_K,
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Multi-scale rendering and evaluation script")
    ...
    parser.add_argument("--gt_folder_path", type=str, required=True,
                       help="Path to ground truth masks (e.g., ../data/lerf_mask/figurines/test_mask)")
    ...
    render_sets(model.extract(args), args.iteration, pipeline.extract(args), 
                args.skip_train, args.skip_test, args.gt_folder_path,
