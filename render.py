...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, segment : bool = False, target = 'scene', idx = 0, precomputed_mask = None):
    ...
    if segment:
        assert target == 'seg' or precomputed_mask is not None and "Segmentation only works with target seg!"
    ...
    with torch.no_grad():
        ...
        if target == 'scene' or target == 'seg' or target == 'xyz':
            gaussians = GaussianModel(dataset.sh_degree)
        if target == 'feature' or target == 'contrastive_feature':
            feature_gaussians = FeatureGaussianModel(dataset.feature_dim)

        scene = Scene(dataset, gaussians, feature_gaussians, load_iteration=iteration, shuffle=False, mode='eval', target=target)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--target', default='scene', const='scene', nargs='?', choices=['scene', 'seg', 'feature', 'contrastive_feature', 'xyz'])
