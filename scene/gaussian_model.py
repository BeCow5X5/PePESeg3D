...
from utils.general_utils import strip_symmetric, build_scaling_rotation, build_inverse_scaling_rotation
...
class GaussianModel:
    ...
    def sample_points_in_gaussians(self, num_samples, sampling_scale_factor=1., mask=None,
        ...
        if mask is not None:
            valid_indices = torch.arange(len(self._xyz), device="cuda")[mask]
            random_indices = valid_indices[random_indices]
        random_indices = random_indices.detach()
        if unique:
            random_indices = torch.unique(random_indices)

        random_points = self.get_xyz[random_indices] # unlike SuGaR, remove gaussian noise applied

        return random_points, random_indices
        
    # points = output of function with _points as input
    # but we set learnable_positions True -> ignore the function
    def reset_neighbors(self, knn_to_track:int=None):
        """
        knn_points
        Input 
        - p1(N, P1, D) : N batch of point clouds
        - p2(N, P2, D) : N batch of point_clouds

        Output
        - knn_dist(N, P1, K) : giving the squared distances to the nearest neighbors, N is batch
        - knn_idx(N, P1, K) : giving the indices of the K nearset neighbors from points
                            in p1 to points in p2
        """
        if knn_to_track is None:
            knn_to_track = 16
        with torch.no_grad():
            knns = knn_points(self.get_xyz[None], self.get_xyz[None], K=knn_to_track)
            self.knn_dist = knns.dists[0]
            self.knn_idx = knns.idx[0] # batch size 1
    ...
    def get_beta(self, x, 
        ...
        """
        # self.beta_mode default 'average'
    ...
    def get_field_values(self, x, gaussian_idx=None, 
        ...
        if gaussian_strengths is None:
            gaussian_strengths = self.get_opacity
        if gaussian_centers is None:
            gaussian_centers = self.get_xyz
        if gaussian_inv_scaled_rotation is None:
            gaussian_inv_scaled_rotation = build_inverse_scaling_rotation(self.get_scaling, self.get_rotation)
        
        if closest_gaussians_idx is None:
            closest_gaussians_idx = self.knn_idx[gaussian_idx]
        print("closest_gaussians_idx shape", closest_gaussians_idx.shape)
        ...
        # transpose of rotation matrix is inverse of rotation matrix
        # calculated Euclidean Norm
        neighbor_opacities = (warped_shift[..., 0] * warped_shift[..., 0]).sum(dim=-1).clamp(min=0., max=1e8)
        # sdf calculation : B * ex(-1/2s^2 * <p-ug * ng>^2 )
        ...
        print("neighbor", neighbor_opacities.shape)
    ...
    def get_cameras_spatial_extent(self, nerf_cameras:CamerasWrapper, return_average_xyz=False):
        if nerf_cameras is None:
            raise ValueError("No nerfmodel is provided")
        
        camera_centers = nerf_cameras.p3d_cameras[0].get_camera_center()
    ...
    def get_smallest_axis(self, return_idx=False):
        """
        Return whole column(x, y, z) of rotation matrix
        If smallest axis is x, then [1, 0, 0] -> the first column of the rotation matrix holds the x, y, z components of that axis
        """
        rotation_matrices = build_rotation(self._rotation)
    ...
    def get_normals(self, estimate_from_points=False, neighborhood_size:int=32):
        ...
        Arg:
            estimate_from_points (bool): Default to False
            neighborhood_size (int): The size of the neighborhood to estimate the normals.
        """
