...
class FeatureGaussianModel:
    ...
    def change_to_segmentation_mode(self, training_args = None, target = 'contrastive_feature', fixed_feature = False):
        
        if target == 'contrastive_feature':
            # Better initialization methods for contrastive feature learning
            if self.feature_dim <= 16:
                # For lower dimensional features, use Xavier uniform initialization
                bound = (6.0 / self.feature_dim) ** 0.5
                self._point_features.data = torch.empty_like(self._point_features.data).uniform_(-bound, bound)
            else:
                # For higher dimensional features, use normalized random initialization
                # This helps with gradient flow and contrastive learning stability
                self._point_features.data = torch.randn_like(self._point_features.data)
                # L2 normalize each feature vector to unit sphere - good for contrastive learning
                self._point_features.data = torch.nn.functional.normalize(
                    self._point_features.data, dim=-1, p=2
                ) * 0.1  # Scale down to prevent vanishing gradients initially
    ...
    def save_ply(self, path, smooth_weights = None, smooth_type = None, smooth_K = None):
        ...
        elif smooth_type == 'traditional': # default
